...
APP_PAGE = {
    ...
    config: {
        ...
        list: SERVER_URL + '/cadastro/sinistro/search/',
        find: SERVER_URL + '/cadastro/sinistro/find/',
        save: SERVER_URL + '/cadastro/sinistro/save/',
        delete: SERVER_URL + '/cadastro/sinistro/delete/',
    },
    ...
    start: function() {
        ...
        // Carregando lista de Seguradoras
        APP_BASE.getSeguradoras();
    },
    ...
    edit: function(id) {
        ...
        SERVER.send(APP_PAGE.config.find, {
            ...
            $('#seguradora').dropdown('set selected', data.row.seguradora);
            $('#acionamento').val(data.row.acionamento);
            $('#apolice').val(data.row.apolice);
            $('#item').val(data.row.item);
            $('#data').val(data.row.data);
            $('#hora').val(data.row.hora);
            $('#brat').val(data.row.brat);
    ...
    save: function() {
        ...
        var data = {
            ...
            seguradora: $('#seguradora').dropdown('get value'),
       ...
       if (data.veiculo == "0" || data.veiculo == null) {
        ...
        SERVER.send(APP_PAGE.config.save, data, function(data) {
            ...
            SENDFILE.save('sinistro', data.id);
